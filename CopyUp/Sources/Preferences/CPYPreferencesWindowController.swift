...
private extension CPYPreferencesWindowController {
    ...
    func selectedTab(_ index: Int) {
        ...
        case 0:
            ...
            generalTextField.textColor = NSColor(resource: .copyup)
        case 1:
            menuImageView.image = NSImage(resource: .prefMenuOn)
            menuTextField.textColor = NSColor(resource: .copyup)
        case 2:
            typeImageView.image = NSImage(resource: .prefTypeOn)
            typeTextField.textColor = NSColor(resource: .copyup)
        case 3:
            excludeImageView.image = NSImage(resource: .prefExcludedOn)
            excludeTextField.textColor = NSColor(resource: .copyup)
        case 4:
            shortcutsImageView.image = NSImage(resource: .prefShortcutOn)
            shortcutsTextField.textColor = NSColor(resource: .copyup)
        case 5:
            updatesImageView.image = NSImage(resource: .prefUpdateOn)
            updatesTextField.textColor = NSColor(resource: .copyup)
        case 6:
            betaImageView.image = NSImage(resource: .prefBetaOn)
            betaTextField.textColor = NSColor(resource: .copyup)
        default: break
        }
    }
